...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans selection:bg-emerald-600 selection:text-white">
      ...
      {/* 🏢 ONAYLI KURUMSAL MAĞAZALAR (PRO SHOWCASE - COMPACT & ELEGANT) */}
      <section id="stores" className="py-12 px-6 bg-gradient-to-b from-slate-50/50 via-white to-slate-50/50 border-b border-slate-200/80">
        <div className="max-w-6xl mx-auto space-y-8">
          <div className="text-center max-w-xl mx-auto space-y-2">
            <span className="inline-flex items-center gap-1.5 text-[10px] font-black text-emerald-700 bg-emerald-50 border border-emerald-200/80 px-3 py-1 rounded-full uppercase tracking-wider">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {stores.map(s => {
              ...
              return (
                <div key={s.id} className="bg-white border border-slate-200/90 hover:border-emerald-500/50 rounded-2xl p-5 space-y-4 shadow-sm hover:shadow-md hover:-translate-y-1 transition-all duration-300 relative overflow-hidden group">
                  {/* Glowing Corner Badge */}
                  <div className="flex items-start justify-between gap-3">
                    <div className="relative">
                      <img src={logo} alt={name} className="w-12 h-12 rounded-xl object-cover border border-slate-100 shadow-sm group-hover:scale-105 transition-transform" />
                  ...
                  <div className="space-y-0.5">
                    <h3 className="font-extrabold text-sm text-slate-900 flex items-center gap-1.5 group-hover:text-emerald-700 transition-colors">
                      <span>{name}</span>
                    </h3>
                    <p className="text-[11px] text-slate-500 font-semibold">{sector}</p>
                  </div>

                  <div className="space-y-1.5 text-[11px] text-slate-600 pt-2.5 border-t border-slate-100/80">
                    ...
                    {phone && (
                      <div className="flex items-center justify-between pt-1">
                        ...
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="bg-emerald-50 hover:bg-emerald-100 text-emerald-700 border border-emerald-200 text-[10px] font-extrabold px-2.5 py-1 rounded-lg transition-colors flex items-center gap-1"
                        >
            ...
            <div className="bg-gradient-to-br from-emerald-900 via-slate-900 to-teal-950 text-white rounded-2xl p-5 flex flex-col justify-between space-y-4 shadow-md border border-emerald-500/30 relative overflow-hidden group hover:scale-[1.01] transition-transform">
              <div className="space-y-2 relative z-10">
                <div className="w-10 h-10 rounded-xl bg-white/10 backdrop-blur-xl border border-white/20 flex items-center justify-center text-emerald-400">
                  <Store className="w-5 h-5" />
                </div>
                <h3 className="text-base font-black text-white leading-snug">İşletmenizi AdaBazaar'a Dahil Edin</h3>
                <p className="text-[11px] text-slate-300 leading-relaxed font-medium">
                  KKTC genelindeki binlerce müşteriye aracısız ulaşın, kurumsal mağaza kimliği ile satış yapın.
                </p>
              </div>

              <button
                onClick={() => setShowApplyModal(true)}
                className="bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-black text-[11px] px-4 py-2.5 rounded-xl transition-all shadow-md flex items-center justify-center gap-1.5 relative z-10"
              >
                <span>{t.storeApply} →</span>
              </button>
            </div>
